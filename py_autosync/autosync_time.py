...
def _prepare_cost(of: GyroSeries, gyro: GyroSeries, initial_offset_ms: float, search_size_ms: float,
                  of_sample_rate_hz: float, gyro_sample_rate_hz: float, lpf_hz: float, interp: bool):
    """Shared setup for the offset search: window + low-pass both signals, build the IMU lookup.

    Returns ``(of_f, cost_fn, keys)`` where ``cost_fn(offset_ms) -> float`` is the cost at a
    candidate offset, or ``None`` if the inputs are empty / the window is empty. Used by both
    :func:`find_offset` and :func:`cost_curve` so they evaluate exactly the same cost.
    """
    ...
    cost_fn = ((lambda o: _cost_interp(o, of_f, gt_ms, gw)) if interp
               else (lambda o: _cost(o, of_f, keys, vals)))
    return of_f, cost_fn, keys


def cost_curve(of: GyroSeries, gyro: GyroSeries, offsets_ms: np.ndarray,
               of_sample_rate_hz: float, gyro_sample_rate_hz: float, lpf_hz: float = 20.0,
               interp: bool = False, initial_offset_ms: float = 0.0,
               search_size_ms: Optional[float] = None) -> np.ndarray:
    """Evaluate the motion-matching cost at each offset in ``offsets_ms`` (for visualization).

    Uses the same windowing / low-pass / lookup as :func:`find_offset`. ``search_size_ms`` controls
    the gyro window and defaults to covering the requested offsets. Returns an array of costs
    (``inf`` where a candidate has too few matches).
    """
    offsets_ms = np.asarray(offsets_ms, dtype=float)
    if search_size_ms is None:
        search_size_ms = float(np.max(np.abs(offsets_ms - initial_offset_ms))) + 1.0
    prep = _prepare_cost(of, gyro, initial_offset_ms, search_size_ms,
                         of_sample_rate_hz, gyro_sample_rate_hz, lpf_hz, interp)
    if prep is None:
        return np.full(offsets_ms.shape, np.inf)
    _, cost_fn, _ = prep
    return np.array([cost_fn(float(o)) for o in offsets_ms])
